...
public class StudentController {
	...
	public @ResponseBody void attachStudentToCurator(@PathVariable String id,
		...
		logger.info("start attache student with id " + id
				+ " with curator, which id is " + curator_id);
		studentService.attachStudentToCurator(Long.parseLong(id),
				Long.parseLong(curator_id));
		logger.info("attaching success");
	}

	@RequestMapping(value = StudURI.ATTACH_STUDENTS_TO_CURATORS, method = RequestMethod.POST)
	public @ResponseBody void attachStudentsToCurators(
			@RequestBody String json, ObjectMapper om) throws IOException {
		JsonNode rootnode = om.readTree(json);
		JsonNode studnode = rootnode.path("studsId");
		JsonNode curnode = rootnode.path("cursId");
		List<Long> stud = om.readValue(studnode.traverse(),
				JsonUtil.listOfLongTypeRef);
		List<Long> cur = om.readValue(curnode.traverse(),
				JsonUtil.listOfLongTypeRef);
	...
	public @ResponseBody List<StudentView> getAllStudents(Principal user) {
		...
		if (role.equals(SpringSecurityRole.CURATOR)) {
			studlist = curatorService.getSupervised(userService.findByLogin(
					user.getName()).getId());
		} else {
	...
	public @ResponseBody String getStudent(@PathVariable("id") String idString,
			ObjectMapper om, Principal user) throws IOException {
		logger.info("caller role searching");
		String role=SecurityUtil.getRole();
		logger.info("caller role - " + role);
		long id = Long.parseLong(idString);
		logger.info("composite student view fetching");
		// CompositeStudentFeedbackView
		// view=service.generateStudentViewForUser(id,role);
		StudentView info = service.findById(id).toView();
		List<FeedbackView> feedbacks;
		if (SecurityUtil.isFeedbackableNotAdmin(role)) {
			feedbacks = service.getFeedbacksForStudentByCurator(id,
					user.getName());
		} else {
			feedbacks = service.getFeedbacksForStudent(id);
		}
	...
	public @ResponseBody String getProjectHistory(@PathVariable("id") long id,
			ObjectMapper om) throws IOException {
		StringWriter sw = new StringWriter();
		JsonGenerator jg = om.getFactory().createGenerator(sw);
		JsonUtil.writeJSONStringObjectArray(jg,
